...
//		CricketTeam []arr = new CricketTeam[tempTeam.size()];
//		for (int i = 0; i < tempTeam.size(); i++) {
//			arr[i] = tempTeam.get(i);
//		}
//		mergeSortByPoints(arr, 0, arr.length-1);
//	}
//	
//	public static void mergeSortByPoints(CricketTeam []arr, int left, int right) {
...
//		mergeSortByPoints(arr, left, mid);
//		mergeSortByPoints(arr, mid+1, right);
//		// allocate temp array
//		CricketTeam []temp = new CricketTeam[(right - left) + 1];
...
////				k++;
////			}
//			temp[k++] = (arr[i].getPoints() < arr[j].getPoints()) ? arr[i++] : arr[j++];
//		}
//		while(i <= mid) {
////			temp[k] = arr[i];
////			i++;
